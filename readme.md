...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[11. Регулярні вирази. Перевірка даних](#task_11)<br />
...
Продемонструвати ефективне (оптимальне) використання регулярних виразів для перевірки коректності (валідації) даних, що вводяться, перед записом в [domain-об'єкти відповідно до призначення кожного поля](#task_07_app) для заповнення [розробленого контейнера](#task_09):
- при зчитуванні даних з текстового файла в автоматичному режимі;
- при введенні даних користувачем в діалоговому режимі.

### Рекомендації

1. [Wiki: Regular expression](https://en.wikipedia.org/wiki/Regular_expression).
2. [Java Tutorials: Regular Expressions](https://docs.oracle.com/javase/tutorial/essential/regex).
3. [Tutorial: Regular expressions in Java](http://www.vogella.com/tutorials/JavaRegularExpressions/article.html).
4. [Регулярные выражения в Java](http://www.quizful.net/post/Java-RegExp).
5. [Eclipse plugin for testing/editing regular expressions](http://myregexp.com/eclipsePlugin.html).
...
1. Використовуючи програми рішень попередніх задач, продемонструвати ефективне (оптимальне) використання регулярних виразів при вирішенні [прикладної задачі](#task_12_app).
...
<span id="task_12_app"></span>
### Прикладні задачі

1. [Кадрове агентство](#task_07_01). Знайти всіх претендентів з досвідом роботи в сфері менеджменту, які не бажають їздити у відрядження.

2. [Кадрове агентство](#task_07_02). Знайти всі вакансії, де потрібні викладачі (педагоги, вчителі) зі стажем не менше 10 років, які знають англійську мову та володіють автомобілем.

3. [Адресна книга](#task_07_03). Знайти всі записи про тих, хто має Харківський номер телефону та є абонентом мереж "lifecell" та "Kyivstar".

4. [Бібліотека](#task_07_04). Знайти всі книги за останні п'ять років у яких хоча б один автор має ініціали "Й. Й." та книги, ISBN яких містить послідовність цифр "...9X9X9..." ("X" - одна будь-яка цифра, "..." - один або декілька символів).

5. [Довідник покупця](#task_07_05). Знайти всі торгові точки, що працюють без вихідних та перерви на обід, що надають для консультації короткий телефонний номер та мають контактні телефони українських операторів мобільного зв'язку.

6. [Поліцейська картотека](#task_07_06). Знайти всіх ув'язнених не молодше 20 років з прізвищем, що починається з голосної та містить комбінацію "ко". Наприклад: Архипенко, Ішкова, Єрмаков, Янковський.

7. [Бюро знайомств](#task_07_07). Знайти всі комбінації пар із різностатевих партнерів з різницею у віці не більше 5 років для ранкових пробіжок в парках Київського району (може бути вказано в довільній формі у вимогах до партнера).

8. [Автостанція](#task_07_08). Знайти всі ранкові рейси у вихідні дні з Харкова до Києва з зупинкою в Полтаві.

9. [Квиткова каса](#task_07_09). Знайти всі вечірні маршрути, що проходять через Харків (транзитні) по понеділках та п'ятницях.

10. [Відділ кадрів](#task_07_10). Знайдено дуже пошкоджений паспорт. Чітко видно останній символ серії паспорта "М", деякі цифри номера: "...8...6..." й деякі літери прізвища власника: "Ар...е..." ("..." означає наявність одного або декількох символів). Знайти всіх співробітників, які могли б володіти цим документом.

11. [Магазин](#task_07_11). Знайти усі товари з актуальним терміном придатності. Дата виробництва, термін придатності (час зберігання або дата закінчення) можуть бути вказані в опису до товару.

12. [Аудіотека](#task_07_12). Знайти аудіозаписи, що стиснуті з мінімальними втратами або без втрат аудіоданих (визначається форматом), в назві або тексті яких згадується святкування Нового Року.

13. [Готель](#task_07_13). Знайти всіх проживаючих клієнтів, що виконують службові доручення поза місцем постійної роботи.

14. [Планувальник](#task_07_14). Знайти всі конференції, що пройшли за останні три роки в Харкові та області з тривалістю не менше доби.

15. [Автосалон](#task_07_15). Знайти всі електромобілі не старіші одного року.

16. [Поліцейська картотека](#task_07_16). Знайти всіх, звільнених з-під варти осіб, з натільним рисунком у вигляді власного імені.

---

<span id="task_13"></span>
...
1. [Java Tutorials: Concurrency](https://docs.oracle.com/javase/tutorial/essential/concurrency/).
2. [Tutorial: Java concurrency (multi-threading)](http://www.vogella.com/tutorials/JavaConcurrency/article.html).

---

<span id="task_14"></span>
...
[Приклад визначення часу обробки даних](https://github.com/oop-khpi/oop-khpi.github.io/blob/master/demo/src/ua/khpi/oop/demo03/Main.java).

---

<span id="task_15"></span>
## 15. Колекції в Java
...
1. Розробити консольну програму для реалізації завдання обробки даних згідно [прикладної області](#task_07_app).

2. Для розміщення та обробки даних використовувати контейнери (колекції) і алгоритми з [Java Collections Framework](https://docs.oracle.com/javase/8/docs/technotes/guides/collections/).

3. Забезпечити обробку колекції об'єктів: додавання, видалення, пошук, сортування згідно розділу [Прикладні задачі л.р. №10](#task_10_app).
...
1. Розробити програму рішення [лабораторної роботи №9](#task_09).
...
1. Кен Арнольд, Джеймс Гослинг. Язык программирования Java<br /> ISBN: [9780132761680](http://www.isbnsearch.org/isbn/9780132761680)<br /> Написана просто, містить багато корисної інформації. Джеймс Гослінг - один з авторів Java. Рекомендується всім, хто починає знайомитися з мовою. Має сенс переглянути і більш досвідченим програмістам.

2. Хорстманн. Java 2. Том 1. Основы<br /> ISBN: [9780135166307](http://www.isbnsearch.org/isbn/9780135166307)<br /> Розглядаються різні аспекти мови - від використання assert до регулярних виразів і NIO.

3. Дэвид Флэнаган. Java в примерах. Справочник<br /> ISBN: [9780596006204](http://www.isbnsearch.org/isbn/9780596006204), 9781492037248<br /> Містить безліч закінчених реалізацій окремих фрагментів технологій - RMI-виклики, доступ до баз через JDBC і т.д. Приклади прості для розуміння, їх можна легко модифікувати.

### Для досвідчених

1. Хорстманн. Java 2. Том 2. Тонкости программирования<br /> ISBN: [9780134177304](http://www.isbnsearch.org/isbn/9780134177304), 9780135167151<br /> Розглядаються більш серйозні теми - багатопоточне програмування, локалізація програми, питання безпеки, робота з XML.

2. Брюс Эккель. Философия Java<br /> ISBN: [9780131872486](http://www.isbnsearch.org/isbn/9780131872486)<br /> Треба прочитати кожному розробнику. Про філософію мови, про її устрій, її підхід до опису світу і вирішення задач. Дуже глибока і корисна для розуміння, що таке Java. Новачкам буде марна.

3. Джошуа Блох. Java. Эффективное программирование<br /> ISBN: [9780134685991](http://www.isbnsearch.org/isbn/9780134685991)<br /> У книзі 78 (у другому виданні) слабко пов'язаних одне з одним невеликих статей, що ілюструють тонкощі Java. Написана практиком з чималим досвідом. Читати, маючи серйозний досвід розробок.

### Про розробку в цілому

1. Стивен Стелтинг, Олав Маассен. Применение шаблонов Java<br /> ISBN: [9780130935380](http://www.isbnsearch.org/isbn/9780130935380)<br /> Присвячена шаблонами проектування - прийомам організації програм, які на практиці довели свою ефективність. Корисна, при проектуванні архітектури програм.

2. Гамма, Хелм, Джонсон, Влиссидес. Приемы объектно-ориентированного проектирования. Патерны проектирования<br /> ISBN: [9780201633610](http://www.isbnsearch.org/isbn/9780201633610), [9780201634983](http://www.isbnsearch.org/isbn/9780201634983)<br /> Читати тільки в оригіналі! Перекладачі доклали всіх зусиль до того, щоб зіпсувати все до неможливості.

3. Эрик Аллен. Типичные ошибки проектирования<br /> ISBN: [9781590590614](http://www.isbnsearch.org/isbn/9781590590614)<br /> Про те, як не треба робити. У ній зібрані типові помилки розробників. Корисна для тих, хто має деякий досвід.

4. Брюс Тейт. Горький вкус Java<br /> ISBN: [9781930110434](http://www.isbnsearch.org/isbn/9781930110434)<br /> Зібрані помилки програмування з використанням J2EE - сервлетів, JSP, EJB і т.п.

### Iнше

1. [Объектно-ориентированное программирование провалилось?](http://blogerator.org/page/oop_why-objects-have-failed)
2. [Объектная парадигма провалилась](https://bugtraq.ru/library/programming/objectshavefailed.html).
3. [Объекты преуспели](https://bugtraq.ru/library/programming/objectshavenotfailed.html).
4. [Я не знаю ООП](https://habr.com/post/147927/).
5. [Рефакторинг, паттерны проектирования, принципы SOLID](https://refactoring.guru/).
6. [Design patterns, anti-patterns, refactoring](https://sourcemaking.com/).
